document.addEventListener('DOMContentLoaded', () => {
    ...
    const dateHeader = document.querySelector('.time-slots-container .date-header');
    const confirmButton = document.getElementById('confirm-btn');

    let selectedService = document.querySelector('.service-card.active')?.dataset.service || null;
    let selectedBarber = document.querySelector('.barber-card.active')?.dataset.barber || null;
    let selectedDate = document.querySelector('.date-option.active')?.dataset.date || null;
    ...
    confirmButton.addEventListener('click', () => {
        if (selectedService && selectedBarber && selectedDate && selectedTime) {
            ...
            const reservationDetails = {
                ...
                serviceName: document.querySelector(`.service-card[data-service="${selectedService}"] .card-title`).textContent,
                serviceImage: document.querySelector(`.service-card[data-service="${selectedService}"] .card-image-placeholder`).style.backgroundImage.slice(5, -2), // Extrae la URL de la imagen
                
                barberId: selectedBarber,
                barberName: document.querySelector(`.barber-card[data-barber="${selectedBarber}"] .card-title`).textContent,
                barberImage: document.querySelector(`.barber-card[data-barber="${selectedBarber}"] .card-image-placeholder`).style.backgroundImage.slice(5, -2),
                
                date: selectedDate,
                dateDisplay: document.querySelector('.date-option.active').textContent, // Ej: Mar 28 Oct
                time: selectedTime,
                duration: document.querySelector('.time-slot.selected .duration')?.textContent || '30 min' // O un valor por defecto
            };
            ...
            alert('Por favor, selecciona un servicio, un barbero, una fecha y una hora.');
        }
    });
